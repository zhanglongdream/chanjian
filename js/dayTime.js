  ...
  MobileCalendar.prototype = {
    ...
    bindEvent: function(type) {
      ...
      function ymCtrlInit() {
        ...
        if (/^\d{4}-\d{1,2}$/.test(_self.trigger.innerHTML)) {
          rs = _self.trigger.innerHTML.match(/(^|-)\d{1,4}/g)
          dateArr.yy = rs[0] - _self.minY
          dateArr.mm = rs[1].replace(/-/g, '') - 1
        } else {
      ...
      function timeCtrlInit() {
        ...
        if (/^\d{2}:\d{2}$/.test(_self.trigger.innerHTML)) {
      ...
      function finishMobileDate(e) {
        ...
        var date_dd =
         ...
         document.querySelector('#start_date').style.color = '#50e3c2'
      ...
      function finishMobileYM(e) {
        ...
        var date_mm =
          ...
          document.querySelector('#start_date').style.color = '#50e3c2'
        date_mm = date_mm > 9 ? date_mm : '0' + date_mm
        _self.trigger.innerHTML = date_yy % passY + _self.minY + '-' + date_mm
        closeMobileCalendar(e)
      }
      //日期时间确认
      function finishMobileDateTime(e) {
        ...
        date_dd = date_dd > 9 ? date_dd : '0' + date_dd
        var time_hh = parseInt(
          Math.round(
            _self.gearDate.querySelector('.time_hh').getAttribute('val')
          )
        )
        time_hh = time_hh > 9 ? time_hh : '0' + time_hh
        var time_mm = parseInt(
          Math.round(
            _self.gearDate.querySelector('.time_mm').getAttribute('val')
          )
        )
        document.querySelector('#start_date').style.color = '#50e3c2'
        time_mm = time_mm > 9 ? time_mm : '0' + time_mm
        _self.trigger.innerHTML = date_yy % passY + _self.minY + '-' + date_mm + '-' + date_dd + ' ' + (time_hh.length < 2 ? '0' : '') + time_hh + (time_mm.length < 2 ? ':0' : ':') + time_mm
        closeMobileCalendar(e)
      }
      //时间确认
      function finishMobileTime(e) {
        ...
        document.querySelector('#start_date').style.color = '#50e3c2'
        time_mm = time_mm > 9 ? time_mm : '0' + time_mm
        _self.trigger.innerHTML = (time_hh.length < 2 ? '0' : '') + time_hh + (time_mm.length < 2 ? ':0' : ':') + time_mm
        closeMobileCalendar(e)
      }
